...
use crate::{errors::{ERR_STREAM_TO_CALLER, ERR_STREAM_TO_SC, ERR_ZERO_DEPOSIT}, storage::{self, Bet, Betslip, BetslipAttributes}};
...
pub trait P2EModule: storage::StorageModule 
    ...
    fn place_bet(&self, 
        ...
        let mut stake = token_amount.clone();
        let mut total_odd = self.calculate_total_odd(bets);
        let mut payout = self.calculate_payout(total_odd, stake);

        let betslip = Betslip {
            creator: caller.clone(),
            bets: bets,
            total_odd: self.calculate_total_odd(bets),
            stake: stake.clone(),
            payout: payout,
    ...
    fn payout_meta(&self, nft: TokenIdentifier, nft_nonce: ManagedBuffer, token: TokenIdentifier){
        ...
        .token_manager()
        .get_token_attributes::<Nfticket<Self::Api>>(num);
        require!(nft_attributes.is_paid != ManagedBuffer::from("true"), "Ticket was already paid!");
        let new_attributes = Nfticket {
            is_paid: ManagedBuffer::from("true"),
            ..nft_attributes
        };
