...
pub trait BetManagerModule: crate::storage::StorageModule 
    ...
    fn place_bet(
        ...
        // Inițializăm selection scheduler dacă nu există
        ...
        // Match bet va folosi selection_scheduler intern
        let (matched_amount, unmatched_amount, updated_bet) = self.match_bet(bet);
        
        // Luăm starea actualizată din selection_scheduler
        let updated_scheduler = self.selection_scheduler(market_id, selection_id).get();
